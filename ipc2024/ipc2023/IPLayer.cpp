...
void CIPLayer::createRequestPacket(int index, int io) {
    ...
    if (addOrUpdate(target_ip, defaultMac, false, false)) {
        ResetARPHeader(io);
        memcpy(arpHeader[io].source_mac, interfaces[index].macAddr, 6);
        memcpy(arpHeader[io].source_ip, interfaces[index].ipAddr, 4);
        memcpy(arpHeader[io].target_ip, target_ip, 4);

        createPacket(0x0001, io);
    }
}
...
void CIPLayer::createReplyPacket(unsigned char* payload_data, int io) {
    ...
    createPacket(0x0002, io);
};

void CIPLayer::createPacket(unsigned short op_code, int io) {
    arpHeader[io].op_code = TO_BIG_ENDIAN_16(op_code);
    SetEthernetDest(arpHeader[io].target_mac, io);
    Send((unsigned char*)&arpHeader[io], ARP_HEADER_SIZE);
};
...
BOOL CIPLayer::Receive(unsigned char* payload_data, int io)
{
    PARP_HEADER data = (PARP_HEADER)payload_data;
    ...
    if (data->op_code == 1) {
        ...
        if (memcmp(data->target_ip, interfaces[io].ipAddr, data->ip_len) == 0 || proxyTable.FindEntryByIP(data->target_ip)) {
            createReplyPacket(payload_data, io); // 수정필요
        }
    }
