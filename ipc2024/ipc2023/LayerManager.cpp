...
void CLayerManager::AddLayer(CBaseLayer* pLayer) // 레이어를 추가
{
	mp_aLayers[m_nLayerCount++] = pLayer;
}

CBaseLayer* CLayerManager::GetLayer(int nindex) // 레이어의 index로 레이어를 찾는 함수
{
	return mp_aLayers[nindex];
}

CBaseLayer* CLayerManager::GetLayer(char* pName) // 레이어의 이름으로 레이어를 찾는 함수
{
...
void CLayerManager::ConnectLayers(char* pcList) // 레이어들을 입력받아 연결 리스트 형태로 만든다.
{
	MakeList(pcList);
